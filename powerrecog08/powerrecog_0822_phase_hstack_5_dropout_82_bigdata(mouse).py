...
for i, filename in enumerate(DATAFILES):
  data = np.genfromtxt(DATADIR + filename, delimiter="\t", dtype= int)[:,2] - (2**10)
...
W3 = weight_variable([HIDDEN_NUM, HIDDEN_NUM]) # weight
b3 = bias_variable([HIDDEN_NUM]) # bias
a3 = tf.nn.tanh(tf.matmul(a2, W3) + b3) # output
W4 = weight_variable([HIDDEN_NUM, HIDDEN_NUM]) # weight
...
for i in range(10000):
  if i%100==0:
    ...
    summary, acc_1 = sess.run([merged,accuracy_1], feed_dict={x: dataset, t: ydata, keep_prob: 1.0})
    print("step = %s acc = %s acc_1 = %s" % (i, acc, acc_1) )
  sess.run(train_step, feed_dict={x: dataset, t: ydata, keep_prob: 0.6})
