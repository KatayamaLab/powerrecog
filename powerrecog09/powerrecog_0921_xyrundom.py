...
ydata_1 = np.empty((0,OUTPUT_NUM), np.float32)
...
#real part
  ...
  r = np.real(np.fft.fft(
    data[0:SAMPLES*TRAIN_NUM].reshape([1600,25])
  ...
  r = np.zeros([1000, OUTPUT_NUM])
  r[:,i] = 1
  ydata_1 = np.append(ydata_1, r, axis = 0)
...
#imaginaly
  ...
  datasetplus = np.hstack((dataset_1 , dataset_2))
  dataset = datasetplus.reshape([(i+1)*80000])
  ydata = ydata_1.reshape([(i+1)*18000])
...
def weight_variable2(shape):
    ...
    initial =  tf.truncated_normal(shape, stddev=0.18)
    return tf.Variable(initial)
...
W2 = weight_variable1([HIDDEN_NUM, 60]) # weight
b2 = bias_variable([60]) # bias
a2 = tf.nn.relu(tf.matmul(a1, W2) + b2) # output
W3 = weight_variable2([60, 40]) # weight
b3 = bias_variable([40]) # bias
a3 = tf.nn.relu(tf.matmul(a2, W3) + b3) # output
a3_drop = tf.nn.dropout(a3, keep_prob)
W4 = weight_variable3([40, 30]) # weight
b4 = bias_variable([30]) # bias
a4 = tf.nn.tanh(tf.matmul(a3_drop, W4) + b4) # output
W5 = weight_variable4([30, OUTPUT_NUM]) # weight
...
for i in range(100000):
  x_batch_1 = np.random.choice(dataset,batch_size)
  x_batch = x_batch_1.reshape(18000,50)
  y_batch_1 = np.random.choice(ydata,batch_size2)
  y_batch = y_batch_1.reshape(18000,18)

  sess.run(train_step, feed_dict={x: x_batch, t: y_batch, keep_prob: 0.5})
  if i%100==0:
    summary, acc = sess.run([merged,accuracy], feed_dict={x: testdataset, t: testydata, keep_prob: 1.0})
    writer.add_summary(summary, i)
    acc_1 = sess.run(accuracy, feed_dict={x: x_batch, t: y_batch, keep_prob: 1.0})
