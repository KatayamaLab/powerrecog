...
# Data files
DATADIR = 'data/'
DATAFILES = ['dryer-low1.txt','fan.txt','heater300.txt','microwave1.txt','none.txt','led.txt','microwave+heater.txt','microwave+LED.txt','microwave+fun.txt','microwave+dryer-low.txt','heater+LED.txt','heater+microwave.txt','heater+fun.txt','heater+dryer-low.txt']
LABELS = ['dryer-low','fan','heater','microwave', 'OFF', 'LED','microwave+heater','microwave+LED','microwave+fun','microwave+dryer-low','heater+LED','heater+microwave','heater+fun','heater+dryer-low']
...
W2 = weight_variable1([HIDDEN_NUM, 100]) # weight
b2 = bias_variable([100]) # bias
a2 = tf.nn.relu(tf.matmul(a1, W2) + b2) # output
W3 = weight_variable2([100, 100]) # weight
b3 = bias_variable([100]) # bias
a3 = tf.nn.relu(tf.matmul(a2, W3) + b3) # output
a3_drop = tf.nn.dropout(a3, keep_prob)
W4 = weight_variable3([100, 100]) # weight
b4 = bias_variable([100]) # bias
a4 = tf.nn.tanh(tf.matmul(a3_drop, W4) + b4) # output
W5 = weight_variable([100, OUTPUT_NUM]) # weight
...
sess = tf.Session()
saver.restore(sess, "/home/pi/powerrecog_su/powerrecog11/Parameter1106")
