...
testdataset_3 = np.empty((0,INPUT1_NUM), np.float32)
...
#imaginaly
  ...
  I = np.imag(np.fft.fft(data[0:SAMPLES*TRAIN_NUM].reshape([TRAIN_NUM,SAMPLES])
    )[:,0:INPUT1_NUM]).astype(np.float32)
...
W2 = weight_variable1([HIDDEN_NUM, HIDDEN_NUM]) # weight
b2 = bias_variable([HIDDEN_NUM]) # bias
a2 = tf.nn.relu(tf.matmul(a1, W2) + b2) # output
W3 = weight_variable2([HIDDEN_NUM, HIDDEN_NUM]) # weight
b3 = bias_variable([HIDDEN_NUM]) # bias
a3 = tf.nn.relu(tf.matmul(a2, W3) + b3) # output
a3_drop = tf.nn.dropout(a3, 0.6)
W4 = weight_variable3([HIDDEN_NUM, HIDDEN_NUM]) # weight
b4 = bias_variable([HIDDEN_NUM]) # bias
a4 = tf.nn.tanh(tf.matmul(a3_drop, W4) + b4) # output
W5 = weight_variable([HIDDEN_NUM, OUTPUT_NUM]) # weight
b5 = bias_variable([OUTPUT_NUM]) # bias
a4_drop = tf.nn.dropout(a4, 0.6)
...
sess = tf.Session()
saver.restore(sess, "/home/pi/powerrecog_su/powerrecog11/Parameter1101")


#learning

for i in range(200):
  loss_val =sess.run([train_step, cross_entropy], feed_dict={x: dataset, t: ydata})
...
while 1:
    ...
    testdataset_3 = np.append(testdataset_3, t, axis = 0)

    t = np.real(np.fft.fft( dataarray[0:SAMPLES*REC_NUM].reshape([REC_NUM,SAMPLES]) )[:,0:INPUT1_NUM]).astype(np.float32) 
    testdataset_4 = np.append(testdataset_4, t, axis = 0)

    testdataset_5 = np.hstack((testdataset_3 , testdataset_4))

    result = sess.run(y, feed_dict={x: testdataset_5})
