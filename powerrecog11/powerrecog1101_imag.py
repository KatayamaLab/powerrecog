...
sess = tf.Session()
saver.restore(sess, "/home/pi/powerrecog_su/powerrecog10/1019imag/Parameter1019imag")


#learning

for i in range(200):
  loss_val =sess.run([train_step, cross_entropy], feed_dict={x: dataset_1, t: ydata})

  if i%100==0:
    summary, acc = sess.run([merged,accuracy], feed_dict={x: testdataset_1, t: testydata})
    writer.add_summary(summary, i)
    acc_1 = sess.run(accuracy, feed_dict={x: dataset_1, t: ydata})
...
while 1:
    ...
    t = np.imag(np.fft.fft( dataarray[0:SAMPLES*REC_NUM].reshape([REC_NUM,SAMPLES]) )[:,0:INPUT_NUM]).astype(np.float32) 
