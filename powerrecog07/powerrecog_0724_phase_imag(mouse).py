...
#imaginaly
  ...
  dataset_1 = np.append(dataset_1, I, axis = 0)

  I = np.imag(np.fft.fft(
    data[SAMPLES*TRAIN_NUM:SAMPLES*(TRAIN_NUM+TEST_NUM)].reshape([TEST_NUM,SAMPLES])
    )[:,0:INPUT_NUM]).astype(np.float32)

  testdataset_1 = np.append(testdataset_1, I, axis = 0)

  I = np.zeros([TRAIN_NUM, OUTPUT_NUM])
  I[:,i] = 1
  y_1data = np.append(y_1data, I, axis = 0)

  I = np.zeros([TEST_NUM, OUTPUT_NUM])
  I[:,i] = 1
  testy_1data = np.append(testy_1data, I, axis = 0)
