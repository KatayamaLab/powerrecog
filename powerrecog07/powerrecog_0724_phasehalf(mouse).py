...
ydata = np.empty((0,OUTPUT1_NUM), np.float32)
testydata = np.empty((0,OUTPUT1_NUM), np.float32)
dataset_1 = np.empty((0,INPUT1_NUM), np.float32)
testdataset_1 = np.empty((0,INPUT1_NUM), np.float32)
y_1data = np.empty((0,OUTPUT1_NUM), np.float32)
testy_1data = np.empty((0,OUTPUT1_NUM), np.float32)
dataset_2 = np.empty((0,INPUT1_NUM), np.float32)
testdataset_2 = np.empty((0,INPUT1_NUM), np.float32)
y_2data = np.empty((0,OUTPUT1_NUM), np.float32)
testy_2data = np.empty((0,OUTPUT1_NUM), np.float32)
...
#real part
  ...
  r = np.zeros([TRAIN_NUM, OUTPUT1_NUM])
  r[:,i] = 1
  y_1data = np.append(y_1data, r, axis = 0)

  r = np.zeros([TEST_NUM, OUTPUT1_NUM])
  r[:,i] = 1
  testy_1data = np.append(testy_1data, r, axis = 0)
...
#imaginaly
  ...
  testdataset_2 = np.append(testdataset_1, I, axis = 0)

  I = np.zeros([TRAIN_NUM, OUTPUT1_NUM])
  I[:,i] = 1
  y_2data = np.append(y_2data, I, axis = 0)

  I = np.zeros([TEST_NUM, OUTPUT1_NUM])
  I[:,i] = 1
  testy_2data = np.append(testy_2data, I, axis = 0)

dataset = np.hstack((dataset_1 , dataset_2))
testdataset = np.hstack((testdataset_1 , testdataset_2))
ydata = np.hstack((y_1data , y_2data))
testydata = np.hstack((testy_1data , testy_2data))
...
correct_prediction = tf.equal(tf.argmax(y,1), tf.argmax(r,1),  tf.argmax(I,1))
...
for i in range(3000):
  if i%100==0:
    summary, acc = sess.run([merged,accuracy], feed_dict={x: testdataset_1, t: testy_1data})
    writer.add_summary(summary, i)
    acc_1 = sess.run(accuracy, feed_dict={x: dataset_1, t: y_1data})
    print("step = %s acc = %s acc_1 = %s" % (i, acc, acc_1) )
  sess.run(train_step, feed_dict={x: dataset_1, t: y_1data})

correct_prediction = tf.equal(tf.argmax(y,1), tf.argmax(t, 1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print(sess.run(accuracy, feed_dict={x: testdataset_1, t: testy_1data}))
